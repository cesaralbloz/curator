...
public class QuorumConfigBuilder {
	...
	public QuorumConfigBuilder(InstanceSpec... specs) {
		...
		instanceSpecs = fromRandom ? ImmutableList.of(InstanceSpec
				.newInstanceSpec()) : ImmutableList.copyOf(specs);
	}
	...
	public QuorumPeerConfig buildConfig(int instanceIndex) throws Exception {
		...
		if (isCluster) {
			Files.write(Integer.toString(spec.getServerId()).getBytes(),
					new File(spec.getDataDirectory(), "myid"));
		}

		Properties properties = new Properties();
		properties.setProperty("initLimit", "10");
		properties.setProperty("syncLimit", "5");
		properties.setProperty("dataDir", spec.getDataDirectory()
				.getCanonicalPath());

...
/**
 * manages an internally running ensemble of ZooKeeper servers. FOR TESTING
 * PURPOSES ONLY
 */
public class TestingCluster implements Closeable {
	...
	/**
	 * Creates an ensemble comprised of <code>n</code> servers. Each server will
	 * use a temp directory and random ports
	 *
	 * @param instanceQty
	...
	public TestingCluster(Collection<InstanceSpec> specs) {
		...
		ImmutableList.Builder<TestingZooKeeperServer> serverBuilder = ImmutableList
				.builder();
	...
	/**
	 * Shutdown the ensemble, free resources, etc. If temp directories were
	 * used, they are deleted. You should call this in a <code>finally</code>
	 * block.
	 *
	 * @throws IOException
	 *             errors
	 ...
	 *             errors
	 */
	public InstanceSpec findConnectionInstance(ZooKeeper client)
			throws Exception {
